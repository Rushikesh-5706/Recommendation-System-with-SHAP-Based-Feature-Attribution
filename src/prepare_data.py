...
def download_file(url, dest_path):
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to download {url}. Status code: {status_code}. Error: {e}"
        )
...
def extract_zip(zip_path, extract_to):
    ...
    if (
        ...
        and os.path.isdir(extract_to)
        and len(os.listdir(extract_to)) > 0
    ):
        print(
            f"Directory {extract_to} already exists and is not empty. Skipping extraction."
        )
        return
...
def process_data(raw_data_dir, processed_data_dir):
    ...
    if not os.path.exists(ratings_file) or not os.path.exists(movies_file):
        raise FileNotFoundError(
            "Raw data files not found. Ensure extraction was successful."
        )
    ...
    if list(ratings_df.columns) != expected_rating_cols:
        raise ValueError(
            f"Expected rating columns {expected_rating_cols}, found {list(ratings_df.columns)}"
        )
    if list(movies_df.columns) != expected_movie_cols:
        raise ValueError(
            f"Expected movie columns {expected_movie_cols}, found {list(movies_df.columns)}"
        )
    ...
    item_names = {
        str(idx): str(movie_id_to_title.get(iid, f"Unknown Item {iid}"))
    ...
    splits = ratings_df.groupby("user_idx", group_keys=False).apply(
    ...
    train_df[cols_to_save].to_csv(
        os.path.join(processed_data_dir, "train.csv"), index=False
    )
    test_df[cols_to_save].to_csv(
