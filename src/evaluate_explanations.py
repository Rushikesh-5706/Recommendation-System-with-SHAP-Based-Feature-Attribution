...
def compute_faithfulness(
    model, test_df, train_interactions, n_items, background_data, device
):
...
def compute_consistency(
    model, train_df, test_df, train_interactions, n_items, background_data, device
):
    ...
    for u_a, u_b, idx_a, idx_b in tqdm(pairs, desc="Consistency Pairs"):
        ...
        top3_a = set(
            [
                x[0]
                for x in sorted(
                    [(i, shap_a[i]) for i in interacted_a],
                    key=lambda x: x[1],
                    reverse=True,
                )[:3]
            ]
        ...
        top3_b = set(
            [
                x[0]
                for x in sorted(
                    [(i, shap_b[i]) for i in interacted_b],
                    key=lambda x: x[1],
                    reverse=True,
                )[:3]
            ]
