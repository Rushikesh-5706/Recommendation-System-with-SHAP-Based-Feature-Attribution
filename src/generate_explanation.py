    ...
    def predict_fn(X):
        ...
        with torch.no_grad():
            ...
            for row in X:
                ...
                gmf_user_agg = model.gmf_item_emb(items_t).mean(
                    dim=0, keepdim=True
                )  # (1, emb_dim)
                mlp_user_agg = model.mlp_item_emb(items_t).mean(
                    dim=0, keepdim=True
                )  # (1, emb_dim)
                ...
                # MLP branch
                mlp_input = torch.cat(
                    [mlp_user_agg, mlp_item_vec], dim=1
                )  # (1, 2*emb_dim)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate SHAP explanation for a user-item pair."
    )
    ...
    background_user_idxs = random.sample(
        train_user_keys, min(100, len(train_user_keys))
    )
    ...
    for itm, s_val in top_contributors_idx:
        ...
        top_contributors.append(
            {"item_id": int(raw_id), "item_name": name, "shap_value": float(s_val)}
        )
    ...
    if len(contributor_titles) == 0:
        explanation = (
            f"We recommend '{target_title}' based on your general viewing history."
        )
