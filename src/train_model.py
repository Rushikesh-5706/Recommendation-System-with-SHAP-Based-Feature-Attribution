...
def evaluate_baseline(baseline, test_df, train_interactions):
    ...
    for u in tqdm(test_users, desc="Baseline Eval"):
        ...
        hits.append(hit_count / 10.0) # Precision@10: fraction of recommended items that are in test set
...
def evaluate_neumf(model, test_df, train_interactions, all_items, device):
    ...
    with torch.no_grad():
        for u in tqdm(test_users, desc="NeuMF Eval"):
            ...
            users_tensor = torch.tensor([u] * len(eval_items), dtype=torch.long).to(device)
...
def main():
    ...
    train_interactions_dict = train_df.groupby('user_idx')['item_idx'].apply(list).to_dict()
    # Also keep a set version for fast exclusion checking
    train_interactions_set = {u: set(items) for u, items in train_interactions_dict.items()}
    ...
    dataset_tuples = []
    for row in tqdm(pos_train_df.itertuples(index=False), desc="Negative Sampling", total=len(pos_train_df)):
        u = row.user_idx
        i = row.item_idx
        ...
        if available_negatives:
            sampled = random.sample(available_negatives, min(neg_samples, len(available_negatives)))
    ...
    # Evaluate NeuMF
    neumf_metrics = evaluate_neumf(model, test_df, train_interactions_set, all_items, device)
